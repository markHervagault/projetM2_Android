...
public class DroneManager extends MapItem
{
    ...
    {
        if (_selectedDrone.getStatus() != EDroneStatus.EN_PAUSE)
            return;
    ...
    {
        if (!(_selectedDrone.getStatus() == EDroneStatus.EN_MISSION || _selectedDrone.getStatus() == EDroneStatus.RETOUR_BASE))
            return;
    ...
    {
        //Mise a jour du drone sur la istic.m2.ila.firefighterapp.map seulement si le drone existe deja en BDD
